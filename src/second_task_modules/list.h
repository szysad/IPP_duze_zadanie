...
typedef struct {
    ...
    _LNode *lastGet; ///< wskaźnik na ostatni zwracany element listy.
    size_t lastGetIndex; ///< ostatnio użyty drugi parametr, podany przy użyciu funkcji @see List_get
...
/**
 * @brief resetuje listę.
 * Resetowanie oznacza usunięcie jej węzłów i przywrócenie jej parametry z momentu inicjalizacji.
 * @param[in,out] list : lista do zresetowania.
 */
...
/**
 * @brief wypisuje listę.
 * @param[in] list : wskaźnik na listę do wypisania.
 * @param[in] printFunction : wskaźnik na funkcję wypisującą wartości węzłów listy.
 */
void List_print(List *list, void (*printFunction)(void *value));

/**
 * @brief zwraca wskaźnik na konkrety w kolejności element listy.
 * @param list : wskaźnik na listę której element pobieramy.
 * @param index : reprezentacja numeru elementu w kolejności listy.
 * Dla indexu większego niż liczba elementów w liście program zakończy się błędem.
 * @return wskaźnik na element, który znajduje się w liście na podanym miejscu.
 */
